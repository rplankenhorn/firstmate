...
| `fm-launch-send-lib.sh`  | Shared launch-command delivery for a fresh worker pane: gate on shell readiness, then source the command from a file so the typed line never scales with the brief (chunked writes stay as defense in depth) |
