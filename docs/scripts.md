...
| `fm-launch-send-lib.sh`  | Shared launch-command delivery for a worker pane: clear the input a departed agent left unread when the pane is reused, gate on shell readiness, then source the command from a file so the typed line never scales with the brief (chunked writes stay as defense in depth) |
