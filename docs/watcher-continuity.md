...
`tests/fm-watcher-lock.test.sh` covers verified-successor attach, recovery publication before stale-lock removal, the typed self-eviction failure, bounded and successor-linked lifecycle rows, a SIGSTOP counterfactual that distinguishes a live PID from a stale beacon before classifying termination, and a stale `.steal` chain reclaimed in place, bounded and non-recursive, without consulting or forming a deeper `.steal.steal` level — the unbounded recursion that once overflowed bash's fork stack and segfaulted the watcher during process-event reconcile.
