...
state/               runtime records and signals; gitignored
  ...
  <id>.launch        the exact launch command the pane's shell sources, so the bytes typed into the pane never scale with the brief; written only by bin/fm-spawn.sh, removed by teardown
